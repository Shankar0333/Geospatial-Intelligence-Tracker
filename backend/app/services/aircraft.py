...
from app.models.schemas import AircraftRoutePoint, AircraftState, HeatmapBin
...
class AircraftService:
    ...
    async def refresh(self) -> None:
        ...
        for row in rows:
            ...
            self._routes[state.icao24] = self._routes[state.icao24][-100:]

    async def run_scheduler(self, interval_s: int = 10) -> None:
        while True:
            await self.refresh()
    ...
    def _generate_sample_rows(self) -> list[list]:
        ...
        for row in base:
            row[5] += random.uniform(-0.8, 0.8)
            row[6] += random.uniform(-0.8, 0.8)
            row[10] = (row[10] + random.uniform(-12, 12)) % 360
        return base
    ...
    def build_heatmap(self, precision: int = 1) -> list[HeatmapBin]:
        ...
        for state in self._states.values():
            ...
            bins[(lat, lon)] += 1
        return [HeatmapBin(latitude=lat, longitude=lon, count=count) for (lat, lon), count in bins.items()]
